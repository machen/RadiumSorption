...
f3, ax3 = plt.subplots(2,2,sharex='col',figsize=(7.3,5.5))
...
markerStyles = ['o','^','s','p']
lineStyles = ["-","--","-.",":"]
fhyPal = sns.color_palette("Reds_d",4)#sns.cubehelix_palette(n_colors=4,dark=0.3,start=0.2,light=0.8,gamma=1.3,rot=0.2)
montPal = sns.color_palette("Greens_d",4)#sns.cubehelix_palette(n_colors=4,dark=0.3,start=-0.2,light=0.8,gamma=1.3,rot=0.2)
goePal = sns.color_palette("Oranges_d",4) #sns.cubehelix_palette(n_colors=4,dark=0.3,start=0,light=0.8,gamma=1.3,rot=0.2)
...
for i in range(len(pHvals)):
    ...
    if not fhySub.empty:
        ...
        Cs = fhySub.ix[:,'Cs (Bq/g)'].values/SA["fhy"]
        sCw = fhySub.ix[:,'sCw (Bq/mL)'].values
        sCs = fhySub.ix[:,'sCs (Bq/g)'].values/SA["fhy"]
    ...
    if not montSub.empty:
        ...
        Cs = montSub.ix[:,'Cs (Bq/g)'].values/SA["namont"]
        sCw = montSub.ix[:,'sCw (Bq/mL)'].values
        sCs = montSub.ix[:,'sCs (Bq/g)'].values/SA["namont"]
        [slope,inter,rval,pval,stdErr] = linregress(Cw,Cs)
        out = np.array([Cw,Cs,sCw,sCs,np.polyval([slope,inter],Cw)])
        out = pd.DataFrame(np.transpose(out), columns = [pHs+' Cw',pHs+' Cs',pHs+' sCw',pHs+' sCs',pHs+' Fit'])
    ...
    if not goeSub.empty:
        ...
        Cs = goeSub.ix[:,'Cs (Bq/g)'].values/SA["goe"]
        sCw = goeSub.ix[:,'sCw (Bq/mL)'].values
        sCs = goeSub.ix[:,'sCs (Bq/g)'].values/SA["goe"]
        [slope,inter,rval,pval,stdErr] = linregress(Cw,Cs)
        out = np.array([Cw,Cs,sCw,sCs,np.polyval([slope,inter],Cw)])
        out = pd.DataFrame(np.transpose(out), columns = [pHs+' Cw',pHs+' Cs',pHs+' sCw',pHs+' sCs',pHs+' Fit'])
    ...
    if not pyrSub.empty:
        ...
        Cs = pyrSub.ix[:,'Cs (Bq/g)'].values/SA["pyr"]
        sCw = pyrSub.ix[:,'sCw (Bq/mL)'].values
        sCs = pyrSub.ix[:,'sCs (Bq/g)'].values/SA["pyr"]
        [slope,inter,rval,pval,stdErr] = linregress(Cw,Cs)
        out = np.array([Cw,Cs,sCw,sCs,np.polyval([slope,inter],Cw)])
        out = pd.DataFrame(np.transpose(out), columns = [pHs+' Cw',pHs+' Cs',pHs+' sCw',pHs+' sCs',pHs+' Fit'])
...
#
fhyOutput.to_csv('MasterTablePlots\\FhyIsothermDataSA.csv')
goeOutput.to_csv('MasterTablePlots\\GoeIsothermDataSA.csv')
montOutput.to_csv('MasterTablePlots\\MontIsothermDataSA.csv')
pyrOutput.to_csv('MasterTablePlots\\PyrIsothermDataSA.csv')